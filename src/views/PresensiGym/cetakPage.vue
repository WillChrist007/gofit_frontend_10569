            ...
            <div class="col-md-12">
                <div class="card border-0 rounded shadow">
                    <div class="card-body" id="print-form">
                        <template v-for="(booking_gym, id) in booking_gyms" :key="id">
                            <h4>GoFit</h4>
                            <p>Jl. Centralpark No. 10 Yogyakarta</p>
                            <br>
                            <h4>STRUK PRESENSI GYM</h4>
                            <p>Nomor Struk : {{ booking_gym.nomor_booking }}</p>
                            <p>Tanggal : {{ booking_gym.waktu_presensi }}</p>
                            <p>Waktu Gym : {{ booking_gym.waktu_gym }}</p>
                            <br>
                            <h4>Member</h4>
                            <template v-for="(user, id) in users" :key="id">
                                <p v-if="user.id_member === booking_gym.id_member">Nama : {{ user.nama }}</p>
                            </template>
                            <template v-for="(member, id) in members" :key="id">
                                <p v-if="member.id === booking_gym.id_member">Id Member : {{ member.id_member }}</p>
                            </template>
                        </template>
                    </div>
                    <button type="button" class="btn btn-primary m-2" @click="printForm">Cetak Struk</button>
                    <router-link :to="{ name: 'kasir.presensigym.index' }" class="btn btn-warning m-2">
                        Kembali
                    </router-link>
                </div>
            </div>
    ...
    export default {
        setup() {
            ...
            onMounted(() => {
                ...
                axios.get('https://200710569.gofit.backend.given.website/api/bookingGym/'+ id,)
                    .then(response => {
